    ...
    def test_postgres_dispatch_cap_skips_published_and_cooling_prefixes(self):
        ...
        self.runs.write_text(
            json.dumps(
                [
                    {
                        "displayTitle": "Release postgres 15.14.1.160",
                        "status": "completed",
                        "conclusion": "failure",
                        "createdAt": "2000-01-01T00:00:00Z",
                        "updatedAt": "2999-01-01T00:00:00Z",
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("already published as postgres-15.14.1.159", result.stdout)
        ...
        self.assertEqual(
            ...
            [
                ...
                "--ref main -f service=postgres -f version=15.14.1.161 -f force=false",
                "workflow run service-release.yml --repo supabase/slim-services "
                "--ref main -f service=postgres -f version=17.6.1.159 -f force=false",
                "workflow run service-release.yml --repo supabase/slim-services "
                "--ref main -f service=postgres -f version=17.9.0.028-orioledb -f force=false",
            ],
        )
