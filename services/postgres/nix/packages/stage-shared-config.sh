...
# generated by the Nix dockerTools image. Probe with the real server binary and fall
# back to 'C' only where the selected runtime cannot resolve the locale;
# database collation follows the selected upstream Dockerfile's initdb contract.
