...
let
  ...
  configBundle = stdenv.mkDerivation {
    ...
    installPhase = ''
      ...
      #     Dockerfile.slim; stage-shared-config.sh probes and falls back to
...
stdenv.mkDerivation {
  ...
  nativeBuildInputs = lib.optionals stdenv.isLinux [ patchelf file python3 binutils ];
  ...
  postFixup =
    lib.optionalString stdenv.isLinux ''
      PORTABLE_POSTGRES_ROOTFS="$out" \
        ...
        PORTABLE_POSTGRES_COMPILER_LIBGCC="${if stdenv.cc.cc ? libgcc then stdenv.cc.cc.libgcc else lib.getLib stdenv.cc.cc}" \
