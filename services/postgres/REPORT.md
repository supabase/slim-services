...
--icu-locale=en_US.UTF-8`). PostgreSQL enters the pinned bundled glibc through
its launcher, which points `LOCALE_ARCHIVE` at the bundled en_US.UTF-8 archive
and pins `LANG`/`LANGUAGE`/`LC_ALL` to the image values. Image tooling
(busybox/bash) uses the separate system locale archive generated with the base
image's Debian `localedef`; `stage-shared-config.sh` probes with the real
`postgres -C` binary and appends an `lc_* = 'C'` fallback only when the
selected runtime cannot resolve the locale (database collation stays
ICU-provider either way).
