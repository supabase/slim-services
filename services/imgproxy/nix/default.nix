...
let
  ...
  sourceOwner = if builtins.length sourceRepositoryParts == 2 then
    lib.elemAt sourceRepositoryParts 0
  else
    throw "imgproxy: sourceRepository must be OWNER/REPOSITORY";
  sourceRepo = if builtins.length sourceRepositoryParts == 2 then
    lib.elemAt sourceRepositoryParts 1
  else
    throw "imgproxy: sourceRepository must be OWNER/REPOSITORY";
  portableVips = (pkgs.vips.override {
    ...
    mesonFlags = (old.mesonFlags or [ ]) ++ (map (name: lib.mesonEnable name false) [
      "cfitsio"
      "fftw"
      "archive"
      "jpeg-xl"
  ...
  '';
  licenseDeps = [
    { name = "libvips"; version = "8.16.1"; spdx = "LGPL-2.1-or-later"; src = portableVips.src; }
    { name = "glib"; spdx = "LGPL-2.1-or-later"; src = pkgs.glib.src; }
    { name = "libheif"; spdx = "LGPL-3.0-or-later"; src = pkgs.libheif.src; }
    { name = "libaom"; spdx = "BSD-2-Clause"; src = pkgs.libaom.src; }
    { name = "libde265"; spdx = "LGPL-3.0-or-later"; src = pkgs.libde265.src; }
    { name = "x265"; spdx = "GPL-2.0-or-later"; src = pkgs.x265.src; }
    { name = "libvmaf"; spdx = "BSD-2-Clause-Patent"; src = pkgs.libvmaf.src; }
    { name = "libjpeg"; spdx = "IJG"; src = pkgs.libjpeg.src; }
    { name = "libspng"; spdx = "BSD-2-Clause"; src = pkgs.libspng.src; }
    { name = "libwebp"; spdx = "BSD-3-Clause"; src = pkgs.libwebp.src; }
    { name = "cgif"; spdx = "MIT"; src = pkgs.cgif.src; }
    { name = "libexif"; spdx = "LGPL-2.1-or-later"; src = pkgs.libexif.src; }
    { name = "libimagequant"; spdx = "GPL-3.0-or-later"; src = pkgs.libimagequant.src; }
    { name = "lcms2"; spdx = "MIT"; src = pkgs.lcms2.src; }
    { name = "expat"; spdx = "MIT"; src = pkgs.expat.src; }
    { name = "zlib"; spdx = "Zlib"; src = pkgs.zlib.src; }
    { name = "libffi"; spdx = "MIT"; src = pkgs.libffi.src; }
    { name = "pcre2"; spdx = "BSD-3-Clause"; src = pkgs.pcre2.src; }
    { name = "cacert"; spdx = "MPL-2.0"; src = cacertLicense; }
  ] ++ lib.optionals pkgs.stdenv.isDarwin [
    { name = "libcxx"; spdx = "Apache-2.0 WITH LLVM-exception"; src = pkgs.libcxx.src; }
    { name = "libresolv"; spdx = "APSL-1.0"; src = pkgs.darwin.libresolv.src; }
    { name = "libiconv"; spdx = [ "BSD-2-Clause" "BSD-3-Clause" "APSL-1.0" ]; src = pkgs.libiconv.src; }
    { name = "libintl"; spdx = "LGPL-2.1-or-later"; src = pkgs.gettext.src; }
    ...
    { name = "glibc"; spdx = "LGPL-2.1-or-later"; src = pkgs.glibc.src; }
    { name = "util-linux"; spdx = "LGPL-2.1-or-later AND GPL-2.0-or-later"; src = pkgs.util-linux.src; }
    { name = "libselinux"; spdx = "LicenseRef-Public-Domain"; src = pkgs.libselinux.src; }
    { name = "libnuma"; spdx = "LGPL-2.1-or-later"; src = pkgs.numactl.src; }
    { name = "gcc-runtime"; spdx = "GPL-3.0-or-later WITH GCC-exception-3.1"; src = pkgs.gcc.cc.src; }
  ];
  licenseManifest = pkgs.writeText "imgproxy-dependency-licenses.json" (builtins.toJSON {
    format = 1;
    ...
    dependencies = map (d: (builtins.removeAttrs d [ "src" ]) // { license_dir = "share/licenses/${d.name}"; }) licenseDeps;
  });
  licenseCopyCommands = lib.concatMapStringsSep "\n" (d:
    "copy_licenses ${lib.escapeShellArg d.name} ${lib.escapeShellArg (toString d.src)}"
  ) licenseDeps;
  ...
  imgproxyBin = pkgs.buildGoModule {
    ...
    buildInputs = [ portableVips pkgs.libunwind ];
    env.CGO_ENABLED = 1;
    subPackages = [ "." ];
    ldflags = [ "-s" "-w" ];
    doCheck = false;
  ...
  rootfs = pkgs.stdenv.mkDerivation {
    ...
    dontFixup = true;
    nativeBuildInputs = [ pkgs.file pkgs.python3 pkgs.makeWrapper ]
