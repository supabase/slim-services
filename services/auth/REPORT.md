...
Auth is the first service on the host-native contract (HOST_NATIVE_ARTIFACTS.md):
...
stage supplies the CA bundle, passwd/group, the `gotrue` symlink, and
static busybox `sh`/`wget`).
